...
- **`releaseExpiredFiles`** settles lapsed reservations in a transaction of its own — triggered only when an upload path's own transaction has been rejected with `STORAGE_LIMIT_EXCEEDED`, and followed by a single retry of that transaction if rows were actually freed. Keeping it separate is the whole point: sharing a transaction with the reservation would let the `checkQuota` rejection roll the refund back too, and the reclaimed headroom would never become visible. It handles up to `MAX_EXPIRED_FILES_PER_SWEEP` (25) expired `pending` rows per call, each on one of three branches in `settleExpiredReservations`: a row whose object is in storage and matches is promoted with an `updateOne` that touches **no** ancestor counters (those bytes were counted at mint); a row whose object is missing gets a `{ _id, userId, status: "pending" }`-scoped `deleteOne` whose `deletedCount` gates the paired inverse `updateAncestorDirectoryStats`; and a row whose lookup failed is skipped, leaving both its reservation and its object intact for the next sweep. Only the deleted rows are returned, so their objects can be dropped **after** the commit; a promoted row's object is a file the user now has. An aborted sweep returns an empty list instead of the collected one: dropping an object for a row the rollback restored would strand a live document naming an absent key.
...
- **`confirmUpload`** deliberately uses **no** transaction. Promoting a pending upload is a single-document compare-and-set, which is already atomic; a session would be dead weight on the upload hot path. Its filter carries `cancelledAt: { $exists: false }` alongside `status: "pending"`, so a cancel that landed after confirm read the row loses nothing: the set fails, the row is re-read, and the caller gets `UPLOAD_CANCELLED` (409) rather than a `ready` row whose object the cancel is about to delete.
...
- `src/services/file.service.js` — `releaseExpiredFiles` and `releaseReservedBytes` (compensating transactions that delete a `pending` row and apply the inverse `updateAncestorDirectoryStats`). `releaseExpiredFiles` runs only after an upload path's transaction has been rejected for quota, and must commit before that transaction is retried; `releaseReservedBytes` undoes a single reservation when the step that followed it failed — presigning, the stream to R2, or the promoting transaction itself, which includes a quota rejection. Both swallow their own errors: a refund that cannot commit is warn-logged and retried the next time an upload is rejected for quota, never surfaced as the caller's error.
