...
The sweep settles the row on the first storage-usage read or quota rejection after that deadline passes — refunding the bytes and dropping the object if the bytes never landed, promoting the row if they did — so the quota returns in roughly six minutes rather than the twenty to sixty-five minutes a lapsed reservation otherwise runs to.
...
Seven details carry the design:

- **On the upload path it runs on rejection, not on arrival.** Both `initiateUpload` and `uploadFileFromServer` open their quota-checked transaction first; only a `STORAGE_LIMIT_EXCEEDED` triggers the sweep, and only if the sweep actually freed rows is that transaction retried — exactly once, after which the real error stands. A user with headroom pays nothing on the upload path at all; a genuinely full user pays one indexed query.
- **It also runs on every storage-usage read.** `getStorageUsage` awaits `settleExpiredPendingFiles(userId)` — the same sweep plus the post-commit object removal — before it reads the root size, so a lapsed or cancelled reservation does not sit in `used` indefinitely for a user who never hits their limit. The sweep swallows its own errors, so a fault there leaves `used` at its current figure rather than failing the read.
...
- **Capped at `MAX_EXPIRED_FILES_PER_SWEEP` (25) per call.** A user with a backlog drains it across several usage reads or rejected uploads instead of paying for the whole sweep on one request. If the sweep aborts, the returned list is discarded rather than acted on — deleting an object for a row the rollback restored would leave a live document naming a key with nothing behind it.

Cancel is what makes this timely for the case users actually notice. Left alone, a reservation runs to a deadline between twenty and sixty-five minutes out; cancelled, it lapses about six minutes after the row was created, so the next storage-usage read or the next upload that hits the quota can settle it.
