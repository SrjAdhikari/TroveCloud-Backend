...
| Code                 | Typical HTTP | Meaning                                                                                                 | Where thrown                          |
| -------------------- | ------------ | ------------------------------------------------------------------------------------------------------- | ------------------------------------- |
| `FILE_NOT_FOUND`     | 404          | The requested file doesn't exist, doesn't belong to the user, or is an upload that has not been confirmed yet. All read paths filter `status: "ready"`, so a pending upload is indistinguishable from a missing file. Cancel returns it for the same two cases — absent, or owned by someone else — so the endpoint cannot be used to probe for another user's file ids. | `getFile`, `createDownloadUrl`, `updateFile`, `deleteFile`, `confirmUpload`, `cancelUpload` |
...
| `STORAGE_LIMIT_EXCEEDED` | 400      | The upload would push the user's total stored bytes past their `storageLimit` quota. Checked by `checkQuota` against the denormalized root-directory size, inside the transaction that reserves the bytes on the browser path and inside the promoting transaction on the server-side path (where the real size isn't known until the stream ends). This code means a genuine overrun: an unusable limit raises `INVALID_STORAGE_LIMIT` instead. Raising it is also what buys one sweep of the user's lapsed reservations and a single retry. | `initiateUpload`, `uploadFileFromServer` in `file.service.js` |
| `INVALID_STORAGE_LIMIT` | 500       | The user's `storageLimit` is absent, null, or non-numeric, so the quota cannot be evaluated and the upload fails closed. Kept separate from `STORAGE_LIMIT_EXCEEDED` because that code triggers a destructive sweep and a retry, which a configuration fault must never drive; it is raised before the root-directory read, so a misconfigured account costs one comparison rather than a query. `Number.POSITIVE_INFINITY` is a valid limit — the declared exemption — not a missing one. | `checkQuota` in `file.service.js` |
...
| `UPLOAD_ALREADY_CONFIRMED` | 400    | From confirm: the upload is already finished and the stored object no longer matches what was approved. A repeat confirm on an object that still matches is idempotent and returns the document instead. From cancel: there is nothing left to cancel — the row is already `ready`, which is also what a `confirmUpload` that won the race leaves behind. | `confirmUpload`, `cancelUpload` in `file.service.js` |
| `UPLOAD_CANCELLED`   | 409          | Confirm was called for a reservation the client had already cancelled. The row carries `cancelledAt`, and confirm's compare-and-set requires that field to be absent, so a confirm arriving after a cancel cannot promote a row whose object the cancel is about to delete. The upload is not recoverable from the client's side — start a new one. Cancel leaves the object untouched, so if the `PUT` did complete, the sweep promotes the row later and the file appears after all. | `confirmUpload` in `file.service.js` |
| `UPLOAD_IN_PROGRESS` | 409          | A delete was attempted while an upload URL for the file could still be used. Deleting would refund bytes the held URL can still consume, so the request is refused until the window closes. Cancelling the upload (`POST /api/files/:id/cancel`) shortens that window to roughly six minutes from the row's creation; the delete stops being refused once it has passed, by which point the signed URL is dead and a refund is safe. Cancel itself never raises this code — it answers with 404 / 400. | `deleteFile` in `file.service.js`, `deleteDirectory` in `directory.service.js` |
...
| `DRIVE_IMPORT_LIMIT_EXCEEDED` | failed[]     | The per-file cap or the per-request aggregate cap tripped during streaming. Both are environment-configured (currently 100 MB and 200 MB).                       | `streamFileIntoTrove` in `drive.service.js` (pre-flight + post-hoc byte counter)            |
